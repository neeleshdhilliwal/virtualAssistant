    ...
    <div className='w-full h-[100vh] bg-cover flex justify-center items-center' style={{ backgroundImage: `url(${bg})` }} >
      <form className='w-[90%] h-[600px] max-w-[500px] bg-[#00000062] backdrop-blur shadow-lg shadow-black flex flex-col items-center justify-center gap-[20px] px-[20px]' onSubmit={handleSignUp}>
        ...
        <div className='w-full h-[60px] border-2 border-white bg-transparent text-white rounded-full text-[18px] relative flex items-center hover:scale-105 hover:shadow-[0_0_20px_#3b82f6] transition duration-300'>
          <FaUser className='absolute left-[20px] text-white w-[20px] h-[20px]' />
          <input type="text" placeholder='Enter your Name' className='w-full h-full outline-none bg-transparent text-white placeholder-gray-300 px-[50px] rounded-full text-[18px]' required onChange={(e) => setName(e.target.value)} value={name} />
        </div>
        ...
        <button className='w-full h-[60px] mt-[30px] text-black font-semibold bg-white rounded-full text-[19px] hover:scale-105 hover:shadow-[0_0_20px_#3b82f6] transition duration-300 ' disabled={loading}>{loading ? "Loading..." : "Sign Up"}</button>

        <p className='text-white text-[18px] cursor-pointer hover:scale-105 hover:shadow-[0_0_20px_#3b82f6] transition duration-300' onClick={() => navigate("/signin")}>Already have an account ? <span className='text-blue-400'>Sign In</span></p>
      </form>
    </div>
  )
}
